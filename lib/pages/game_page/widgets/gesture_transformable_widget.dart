...
class _GestureTransformableState extends State<GestureTransformable>
  ...
  Matrix4 matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    final maxTranslationLeft = (translationBoundaries.right - matrix.getTranslation().x) / scale;
    final maxTranslationRight = (translationBoundaries.left - matrix.getTranslation().x) / scale;
    final maxTranslationUp = (translationBoundaries.bottom - matrix.getTranslation().y) / scale;
    final maxTranslationDown = (translationBoundaries.top - matrix.getTranslation().y) / scale;

    final boundedTranslationDx = translation.dx >= 0 ?
          (translation.dx > maxTranslationLeft ? maxTranslationLeft : translation.dx) :
          (translation.dx < maxTranslationRight ? maxTranslationRight : translation.dx);

    final boundedTranslationDy = translation.dy >= 0 ?
          (translation.dy > maxTranslationUp ? maxTranslationUp : translation.dy) :
          (translation.dy < maxTranslationDown ? maxTranslationDown : translation.dy);


    final nextMatrix = matrix.clone()
      ..translate(
        boundedTranslationDx,
        boundedTranslationDy,
      );
